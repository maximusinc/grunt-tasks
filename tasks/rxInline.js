...
module.exports = function(grunt) {
    ...
    grunt.registerMultiTask('rxinline', "Replaces <link>, <script> and <img> tags to their inline contents", function() {
    ...
    function html(filepath, fileContent, relativeTo, options){
        ...
        }).replace(/<link.+?(?:rel=["']([^"']+?)["'].*)?\s(?:type=["']([^"']+?)["'].*)?\s(?:inline=["']([^"']+?)["'].*)?\shref=["']([^"']+?)["'].*?\/?>/g, function(matchedWord, rel, type, inline, src){
            var ret = matchedWord;
            //todo check other attributes
            if((inline == null || inline.toLowerCase() == 'true') && !isRemotePath(src) && src.indexOf(options.tag)!=-1 && (rel != null && rel.toLowerCase() == "stylesheet")){
    ...
    function cssInlineToHtml(htmlFilepath, filepath, fileContent, relativeTo, options) {
        ...
        fileContent = fileContent.replace(/url\(["']*([^)'"]+)["']*\)/g, function(matchedWord, imgUrl){
            ...
            grunt.log.debug( 'newUrl: '+newUrl);
            grunt.log.debug( 'newUrl with prefix: '+ options.resInlinePrefix + newUrl);

            absoluteImgurl = absoluteImgurl.replace(/\?.*$/, '');
            if(flag && grunt.file.exists(absoluteImgurl)){
                newUrl = datauri(absoluteImgurl);
                grunt.log.debug( 'newUrl abs: '+newUrl);
            }else{
                if (options.resInlinePrefix) {
                    newUrl = options.resInlinePrefix + newUrl;
                }
                newUrl = newUrl.replace(/\\/g, '/');
            }
